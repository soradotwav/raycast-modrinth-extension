...
export default function VersionInteractionMenu(props: {data: ModChangelogAPIResponse, slug: string, showDetails?: boolean}) {
  return (
    <ActionPanel title={`Options for ${props.data.name}`}>
      {props.showDetails ? <Action.Push
        title={"View Details"}
        icon={Icon.Info}
      ...
      <Action.OpenInBrowser
        ...
        icon={Icon.Download}
      />
      <Action.CopyToClipboard title={"Copy URL to Clipboard"} shortcut={{key: "c", modifiers: ["cmd"]}} content={`https://modrinth.com/mod/${props.slug}/version/${props.data.id}`} />
    </ActionPanel>
  )
}